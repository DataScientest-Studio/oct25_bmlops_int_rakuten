...
def main():
    ...
    print(f"Using KNN model folder: {SM_FOLDER}")
    mlflow.log_param("model_folder", str(SM_FOLDER))

        # ---- Load arrays ----
    product_ids = np.load(SM_FOLDER / "knn_product_ids.npy")
    topk_neighbors = np.load(SM_FOLDER / "knn_top10_neighbors.npy")

    with open(SM_FOLDER / "knn_id_to_index.pkl", "rb") as f:
    ...
    distribution = np.bincount(cat_match_counts, minlength=K+1)
        ...
        # ---- Log Metriken in MLflow ----
    mlflow.log_metric("avg_category_match_score", avg_score)
       # Optional: Verteilung der Matches als separate Metriken
    ...
    mlflow.log_artifact(out_path)  # Evaluation CSV als Artefakt speichern
