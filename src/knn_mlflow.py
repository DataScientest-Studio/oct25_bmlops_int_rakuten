...
def build_knn(num_k=10, combine_weight=0.4):
    """
    Build and persist a KNN-based content recommendation model.

    Steps:
    1. Load environment & resolve paths
    2. Load text and image embeddings from MongoDB
    3. Combine embeddings with a weighting factor
    4. Compute KNN similarity matrix
    5. Save artifacts locally and log parameters and metrics to MLflow

    Parameters:
    - num_k: number of nearest neighbors
    - combine_weight: weight for image vs text embeddings
    """
    # Environment & paths
    sh.load_env_vars()
    ...
    df = dh.load_cursor2("products", ["productid", "text_embed", "embedding_img"])
    ...
    # Log artifacts to MLflow (doesnt work properly)
...
def evaluate_knn(sm_folder, topk_neighbors, product_ids, K=10):
    """
    Evaluate KNN quality by checking how often neighbors
    share the same product category (prdtypecode).
    """
    # Load product categories
    df = dh.load_cursor2("products", ["productid", "prdtypecode"])
    ...
    # Count category matches per product
